...
gender_df = gender_df.withColumn('delta', gender_df.final_female_salary - gender_df.final_male_salary)

# ===== city averages ======
city_avg = df2.groupBy('City').agg(f.avg('new_salary').alias('avg_salary'))

city_avg = city_avg.sort(f.col('avg_salary').desc())
